...
<script type="text/javascript">
    $(function () {
    var chart;
            var chart2 = new Array(${detallePreguntaD.getRowCount()});
            var pregunta = "${detallePreguntaD.getRowsByIndex()[0][0]}".split(" ");
            var max = 70;
            var acom = 0;
            var finali = "";
    ...
    <c:forEach items="${detallePreguntaD.rowsByIndex}" var="encuesta" varStatus="status" >
    chart2[${status.index}] = new Highcharts.Chart({
    chart: {
    renderTo: 'container${encuesta[4]}${status.index}',
    ...
    renderTo: 'grafica',
            ...
            xAxis: {
            categories: [
    <c:forEach items="${detallePreguntaD.rowsByIndex}" var="detalleP" varStatus="status">
        <c:choose>
            <c:when test="${detallePreguntaD.getRowCount()!=status.index+1}">
            '${detalleP[1]}',
            </c:when>
            <c:otherwise>
            '${detalleP[1]}'
            ...
            name: 'Encuesta',
                    data: [
    <c:forEach items="${detallePreguntaD.rowsByIndex}" var="detalleP2" varStatus="status33">
        <c:choose>
            <c:when test="${detallePreguntaD.getRowCount()!=status33.index+1}">
                <c:choose>
...
<div class="hero-unit">
    <div class="row">
        <div id="conte" class="span10">
            <br/>
            <legend>Pregunta: ${detallePreguntaD.getRowsByIndex()[0][11]} ${detallePreguntaD.getRowsByIndex()[0][0]}</legend>
            <ul class="breadcrumb">
                ...
                <li class="active tool" data-placement="top" rel="tooltip" data-original-title="${detallePreguntaD.rowsByIndex[0][0]}">Pregunta ${detallePreguntaD.rowsByIndex[0][3]}</li>
            </ul>
            <br>
            <c:choose>
                <c:when test="${detallePreguntaD.getRowCount()!= 0}">
                    ...
                    <table class="table table-striped table-bordered table-condensed inicial">
                        ...
                        <tbody>
                            <c:forEach items="${detallePreguntaD.rowsByIndex}" var="row" varStatus="iter">
                                <tr>
                                    <c:choose>
                                        <c:when test="${iter.index == 0}">
                                            ...
                                            <td style="text-align: left">   
                                                <c:if test="${iter.index == 0 && (row[4] == 1 || row[4] == 3)}">
                                                    Pregrado Distancia
                                                </c:if>
                                                <c:if test="${iter.index == 0 && row[4]== 2}">
                                                    Catedraticos
                                                </c:if>
                                            </td>
                                            <td>   
                                                <div class="btn-group pull-right">
                                                    ...
                                                    <ul class="dropdown-menu" style="padding-top: 0px;">
                                                        <div id="container${row[4]}${iter.index}" style="min-width: 850px; height: 400px; margin: 0 auto"></div>
                                                    </ul>
                                                </div>
                                            </td>
                                        ...
                                        <c:otherwise>
                                            ...
                                            <td style="text-align: left">
                                                <c:if test="${iter.index == 1 && (row[4] == 1 || row[4] == 3)}">
                                                    Postgrado
                                                </c:if>
                                                <c:if test="${iter.index == 2 && (row[4] == 1 || row[4] == 3)}">
                                                    Pregrado Presencial
                                                </c:if>
                                                <c:if test="${iter.index == 1 && row[4] == 2}">
                                                    Medio Tiempo
                                                </c:if>
                                                <c:if test="${iter.index == 2 && row[4] == 2}">
                                                    Tiempo Completo
                                                </c:if>
                                                <c:if test="${iter.index == 3 && row[4] == 2}">
                                                    Tiempo Parcial
                                                </c:if>
                                            </td>
                                            <td>   
